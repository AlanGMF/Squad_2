        ...
        if name == 'prod_cat_info.csv' or name == 'transactions.csv':
            if name == 'prod_cat_info.csv':
                # Adding and saving column
                 df = transform.df_prod(dic_root[name])
                 save.save_csv(name, df)
            else:
                # Adding and saving column
                df = transform.df_transactions(dic_root[name])
                save.save_csv(name, df)
        else:
            if name == 'customer.csv':
                df = transform.df_customers(dic_root[name])
                save.save_csv(name, df)
            else:
                df = transform.no_change(dic_root[name])
                save.save_csv(name, df)
